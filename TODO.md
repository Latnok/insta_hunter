...
## UX-редизайн интерфейса — 2026-07-23

- [x] Перестроить навигацию вокруг сценария менеджера: кандидаты → блогеры → контент; очередь и настройки объединить в «Систему».
- [x] Завершить RU/EN-локализацию пользовательских текстов, статусов, типов заданий и подтверждений на рабочих экранах.
- [x] Добавить единые состояния пустой выдачи, обработки, успеха, предупреждения и ошибки.
- [x] Упростить поиск кандидатов, убрать ручное добавление и CSV в раскрываемый вторичный блок.
- [x] Сделать состояние обработки и рекомендуемое следующее действие главным элементом карточки кандидата.
- [x] Перестроить подробности кандидата и блогера: профиль, вывод ИИ, материалы, решение и скрытые технические сведения.
- [x] Показывать в реестре блогеров состояние персонального предложения; архивирование перенести во вторичные действия.
- [x] Переименовать «Рилсы» в «Контент», локализовать метрики и сделать карточки целиком открываемыми.
- [x] Сделать очередь страницей «Задачи»: сначала проблемы и активная работа, затем история, с понятными названиями действий.
- [x] Разделить настройки на обзор, автоматизацию, критерии и экспертный режим без изменения формата versioned-конфигурации.
- [x] Добавить одноразовые уведомления после POST-действий и улучшить доступность навигации, форм и drawer.
- [x] Обновить UI/i18n/repository-тесты и проверить все страницы визуально на десктопе 1440 px.

Принятые решения: полный UX-редизайн без изменения бизнес-логики; основной пользователь — менеджер без технической подготовки; полная локализация RU/EN; технические редакторы и логи остаются в разделе «Для специалистов»; приоритет — десктоп.
...
- [x] Развернуть schema-compatible release `0.2.5` и проверить реальные avatar/thumbnail responses через production same-origin endpoints.
...
- [x] После 10 новых решений автоматически ставить низкоприоритетный `propose_criteria` job.
- [x] При наличии новых решений обновлять draft не реже одного раза в 24 часа.
- [x] Требовать от LLM структурированную модель отбора: идеальный профиль, обязательные, желательные и исключающие признаки, веса и объяснение изменений.
- [x] Передавать активную структурированную модель в последующий анализ кандидатов.
- [x] Никогда не активировать LLM-критерии автоматически; сохранять только draft для проверки человеком.
- [x] Запускать автопоиск по уникальным запросам только активной версии с глобальным дневным бюджетом 20 и лимитом 5 на запрос.
- [x] Дедуплицировать scheduler jobs и сохранять lifecycle уже известных аккаунтов.
- [x] Поставить автоматическим jobs приоритет ниже ручных.
...
- [x] Вынести системные инструкции анализа блогера и написания оффера из worker-handlers в единый модуль с безопасными значениями по умолчанию.
- [x] Хранить промпты без новой миграции внутри существующей versioned-конфигурации `criteria_versions.transcript_rules`.
- [x] Показывать активные промпты в больших редактируемых полях на странице Settings.
- [x] Сохранять изменение только как draft и применять его к worker после явного Activate.
- [x] Показывать промпты каждой версии в истории до активации.
- [x] Сохранять пользовательские промпты при создании нового criteria draft через LLM.
...
- [x] Создавать `draft_outreach` атомарно при первом одобрении кандидата.
- [x] Не создавать дубликат при восстановлении блогера из архива.
- [x] Формировать сообщение и отдельное обоснование персонализации по профилю, оценке и рилсам.
- [x] Запретить промпту выдумывать факты и коммерческие условия.
- [x] Хранить черновик независимо от полного LLM-лога и обеспечить job-level идемпотентность.
- [x] Добавить редактирование, утверждение, отклонение и повторную генерацию в карточку блогера.
...
- `npm run check`: успешно, 59 JavaScript-файлов и 14 EJS-шаблонов.
- `npm test`: 54 default-теста успешно; отдельный PostgreSQL-прогон — 32/32.
...
- [x] Логировать каждую provider attempt, а не только итогового победителя: SocialCrawl/ScrapeCreators и Groq URL/file fallback сохраняют отдельные succeeded/failed строки с duration/status/request ID; response payload проходит рекурсивную redaction ключей, Bearer/OpenAI-подобных токенов и чувствительных URL-параметров до записи в `provider_call_logs`.
