...
Справа в шапке находятся два оперативных переключателя:

- **Автопоиск** — включает или выключает фоновый поиск новых кандидатов;
- **Автообработка** — включает или выключает автоматический анализ новых
  аккаунтов.

Переключатели применяются сразу. Зелёное состояние означает **включено**,
серое — **выключено**. Ручной поиск и кнопка **Запустить анализ** доступны
...
Если в шапке включена **Автообработка**, после добавления новых аккаунтов
сервис автоматически ставит в очередь сбор профиля, загрузку материалов,
получение расшифровок и оценку. Если она выключена, аккаунты появятся в списке
без запуска анализа — его можно начать вручную из карточки кандидата.
...
Изменения критериев и параметров на вкладке **Автоматизация** не применяются
немедленно. Кнопка сохранения создаёт **черновик**. Чтобы новая версия начала
использоваться, откройте **Обзор**, проверьте изменения и нажмите
**Сделать рабочей**.

Исключение — переключатели **Автопоиск** и **Автообработка** в шапке. Они
меняют рабочее состояние сразу, не создают черновик и записываются в журнал
аудита.
...
### Быстрые переключатели в шапке

**Автопоиск** управляет только фоновым поиском по запросам из активной версии
критериев. При выключенном автопоиске пользователь по-прежнему может запустить
поиск вручную в разделе **Кандидаты**.

**Автообработка** определяет, запускать ли анализ автоматически для аккаунтов,
которые найдены поиском, добавлены вручную или импортированы из CSV. При
выключенной автообработке кандидат сохраняется без pipeline; для него остаётся
доступна кнопка **Запустить анализ**.

Оба переключателя действуют немедленно и не требуют активации новой версии
настроек.
