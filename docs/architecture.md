...
.github/workflows/        required GitHub Actions CI
```
...
GitHub Actions CI использует locked `npm ci` и disposable PostgreSQL 16 service. Default tests и PostgreSQL suites запускаются раздельно; integration step всегда получает `TEST_DATABASE_URL`, поэтому отсутствие БД не может превратить обязательные suites в skip. Workflow имеет только read-only `contents` permission и не получает production/provider credentials.
