...
- Worker каждые пять минут запускает idempotent scheduler LLM-критериев и discovery; scheduler только создаёт обычные jobs с пониженным приоритетом и защитными дневными лимитами.
